...
<p align="center">
  <a href="#安装与环境配置"><img src="https://img.shields.io/badge/安装-支持-4c78a8" alt="安装"></a>
  <a href="#功能"><img src="https://img.shields.io/badge/视频下载-支持-4c78a8" alt="视频下载"></a>
  <a href="#功能"><img src="https://img.shields.io/badge/字幕翻译-多语言-70ad47" alt="字幕翻译"></a>
  <a href="#功能"><img src="https://img.shields.io/badge/声音克隆-Qwen3--TTS-f28e2b" alt="声音克隆"></a>
  <a href="#安装与环境配置"><img src="https://img.shields.io/badge/硬字幕-烧录-d9534f" alt="硬字幕"></a>
  <a href="#配置翻译模型"><img src="https://img.shields.io/badge/API-可选-7b3294" alt="API"></a>
</p>
...
安装脚本会准备 `skills/video-dubber` 的运行环境。若只想注册 Skill 而不立即执行环境安装：
...
| 依赖 | 用途 | 必需性 |
| --- | --- | --- |
| Python 3.10+ | 运行下载、ASR、翻译和渲染脚本 | 必需 |
| `ffmpeg` / `ffprobe` | 音视频抽取、混音、硬字幕和验证 | 必需 |
| `yt-dlp` | 下载在线视频 | 仅 URL 输入必需 |
| Whisper / `faster-whisper` | 没有平台字幕时的 ASR | 无源字幕时必需 |
| Apple Silicon + MLX | Qwen3-TTS 本地声音克隆 | 克隆配音时推荐 |
...
| 配置项 | 用途 | 是否需要 Key |
| --- | --- | --- |
| `DEEPSEEK_API_KEY` | 默认字幕翻译模型 | 需要 |
| `GEMINI_API_KEY` | Gemini 字幕翻译 | 需要 |
| `OPENAI_API_KEY` | OpenAI 兼容翻译接口 | 需要 |
| `NVIDIA_API_KEY` | NVIDIA 翻译模型或 Riva ASR | 需要 |
| Ollama | 本地字幕翻译 | 不需要 |
| `QWEN3_TTS_MODEL` | 指定本地 Qwen3-TTS 模型目录 | 不需要，但克隆配音需要模型 |
...
不要把 `.env`、浏览器 cookies 或模型访问令牌提交到 Git。没有翻译 API Key 时，任务会保留 `source_raw.srt`，可以由 Agent 或人工完成翻译后继续。

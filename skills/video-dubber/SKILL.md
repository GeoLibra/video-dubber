...
执行优先级：内容完整准确 > 不裁句尾 > 时间线同步 > 自然语速；同时保证过程可续跑、可验证、少返工。
...
- 先把相邻字幕合并成语义段：合并到窗口至少约 3.2s，且估算目标语言能在窗口内说完。只有能确认同一说话人、同一句连续语义且不覆盖关键音效时才合并；说话人未知时不得跨潜在角色边界盲目合并。短句如“好的/拜拜/或者是下周”仅在满足这些边界条件时并入前后句。
- 每个语义段只生成一条字幕和一条 TTS，字幕文本与 TTS 文本完全一致。
- 如果 TTS 仍略长，先检查是否还能安全合并窗口，再局部提高 TTS speed；默认不重新概括或删减译文，不要拉长视频，也不要让语音读 A 而字幕显示 B。
- `--max-atempo` 是建议倍率和报告阈值。默认 `--allow-atempo-overflow`：实际需要更高倍率时继续完整生成，不裁句尾；用户可显式指定 `--no-atempo-overflow`，此时保留 overhang，也仍不得裁尾。
- 验证必须报告：分组数、`max_needed_atempo_ratio`、`max_atempo_ratio`、裁切片段数、`quality_warning`、各语速等级数量、语速突变和对应时间点。
...
- 默认 `--max-atempo 1.6 --allow-atempo-overflow`。`--max-clip-ms` 仅保留为旧参数兼容，配音流程不得据此裁掉句尾。
- 当 TTS 明显长于字幕窗口时，优先保留完整句子；能安全合并就合并，否则按实际所需倍率加速，并在报告中记录。高倍率是需要用户试听的交付提示，不是阻塞输出的错误。
...
风险段补丁流程：
1. 读取 `tts_alignment_report`，找出语速等级为 `notice/obvious/extreme`、相邻倍率突变、overhang 或历史 cropped 的段。
2. 优先检查是否能在同一说话人、同一连续语义范围内安全合并时间窗；默认不删减或概括内容。
3. 只重生受影响的 chunk，复用其他 chunk 重建 timeline。
4. 重新烧录输出视频，确认无裁尾、内容和字幕一致、音视频 duration 与源视频一致；高倍率片段保留在最终交付报告中，不阻断输出。

语速等级：
- `natural`：`<= 1.15x`，正常。
- `notice`：`> 1.15x` 且 `<= 1.30x`，交付报告列出。
- `obvious`：`> 1.30x` 且 `<= 1.50x`，标记明显加速并建议试听。
- `extreme`：`> 1.50x`，优先安全合并窗口；仍无法降低时保留完整内容并输出，重点告知用户。
- 相邻片段倍率变化达到 `0.25x` 也必须记录为语速突变。

已有逐条 TTS chunk 可用 `scripts/realign_existing_chunks.py` 重分组，不需要重新翻译或重新加载模型。只有视频明确为单一讲解者，或已可靠取得说话人标签时，才传 `--assume-single-speaker`；电影、电视剧和多人访谈默认不传：

```bash
python scripts/realign_existing_chunks.py \
  --job-dir "$job_dir" \
  --translations translations_ko.json \
  --chunks-dir qwen3tts_ko_chunks \
  --base-video output_original_ko_target.mp4 \
  --merged-audio merged_tts_ko_target_qwen3tts_v2.wav \
  --output output_cloned_ko_target_qwen3tts_v2.mp4 \
  --report tts_alignment_report_ko_qwen3tts_v2.json \
...
若存在裁尾或字幕/TTS 文本不一致，不能宣称内容完整；必须修复后再交付。高倍率或 `quality_warning` 不阻止最终输出，但必须如实告知用户，不能仅凭“没有截断”宣称自然或质量通过。
